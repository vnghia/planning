...
class System {
 protected:
  System(
      ...
      const Tensor2F& costs, const Tensor2F& arrivals,
      const Tensor2F& departures, const float_type* env_trans_mats,
      ...
      const std::function<bool(const State& states, index_type i,
                               index_type a)>& should_skip_state_action_func,
      const std::function<std::optional<index_type>(
          const State& states, const ConstRowXprAI& s1, const ConstRowXprAI& s2,
          index_type a)>
          next_to_func);
