...
System::System(
    const index_type n_env, const index_type n_cls, const VectorAI& limits,
    const Tensor2F& costs, const Tensor2F& arrivals, const Tensor2F& departures,
    const float_type* env_trans_mats, const reward_func_type& reward_func,
    const std::optional<float_type>& normalized_c,
    const std::function<float_type(const System& system)> normalized_c_func,
    const std::function<bool(const State& states, index_type i, index_type a)>&
        should_skip_state_action_func,
    const std::function<
        std::optional<index_type>(const State& states, const ConstRowXprAI& s1,
                                  const ConstRowXprAI& s2, index_type a)>
        next_to_func)
    : n_env(n_env),
      n_cls(n_cls),
      limits(limits),
      ...
      env_trans_mats(([this, env_trans_mats, n_cls]() {
        {
      ...
      normalized_c(([this, normalized_c, &normalized_c_func]() {
        {
          if (normalized_c) return normalized_c.value();
          return normalized_c_func(*this);
        }
      })()),
      rewards(([this, &reward_func, n_cls]() {
      ...
      env_state_accessible(([this, env_trans_mats, n_cls]() {
      ...
      trans_probs(
          ([this, &should_skip_state_action_func, &next_to_func, n_cls]() {
            ...
            for (index_type i = 0; i < states.sys.n; ++i) {
              ...
              for (index_type a = 0; a < n_cls; ++a) {
                if (should_skip_state_action_func(states, i, a)) continue;
      ...
      state_cls_trans_probs(([this, n_cls]() {
...
void System::step(index_type action) { state_ = step(state_, action); }
